...
report_host = $SLAVE_IP     # Обычно используется для указания IP-адреса или доменного имени, по которому мастер должен подключаться к слейву.
...
relay_log_index = /var/lib/mysql/relay-bin.index # Хранит список всех релей-логов, полезно для управления реплика-цией.
